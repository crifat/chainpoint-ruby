  ...
  module ChpSchema
    module V4
      CHAINPOINT_SCHEMA_V4 = {
              ...
              operation: {
                  ...
                  properties:           {
                      ...
                      op:      {
                          ...
                          type:  'string',
                          enum:  %w[sha-224 sha-256 sha-384 sha-512 sha3-224 sha3-256 sha3-384 sha3-512 sha-256-x2]
                      },
                      anchors: {
                          items:       {
          ...
          required:             %w[@context type proof_id hash hash_received branches],
          title:                'Chainpoint v4 JSON Schema.',
          type:                 'object'
      }.freeze

      def is_valid_v4_schema?(data)
        JSON::Validator.validate(CHAINPOINT_SCHEMA_V4, data)
      end

      def validate_v4_schema!(data)
        valid, error = true, nil
        begin
          JSON::Validator.validate!(CHAINPOINT_SCHEMA_V4, data)
        rescue JSON::Schema::ValidationError => e
          valid, error = false, e.message
        end
